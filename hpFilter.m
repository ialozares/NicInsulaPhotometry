function [hpf_dats, mov_ave] = hpFilter(ts, ddf)
%% highpass filtering
hp_window = 90; % Convolution window size in secs
plot_hp = col_rep(1); % color vector [# # #] OR col_rep(#) (leave empty ([]) if no plot desired)

ts_beg = ts(1); ts_end = ts(end);
for hp = 1:length(ddf)
[~,i_wide] = max(ts_end - ts_beg);
